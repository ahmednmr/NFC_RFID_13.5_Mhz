...
ret_codes_t idle_state(void)
{
	...
	if(read_tag_status==Tag_exist)
	{
		...
		if(((tag_data_struct.tag_company_id)!=Company_ID)&&((tag_data_struct.tag_id)!=MASTER_CARD_ID)&&((tag_data_struct.tag_id)!=SETTINGS_CARD_ID)&&((tag_data_struct.tag_id)!=LOAD_CARD_ID))
		{
		...
		case TAG_operation_done:
			...
			if(tag_data_struct.tag_id==MASTER_CARD_ID)
			{
				...
				for(U8 i=0;i<2;i++)
				{
					...
					LCD_vidsend_sentance("Master Card");
					_delay_ms(1000);
			...
			else if(tag_data_struct.tag_id==SETTINGS_CARD_ID)
			{
				return_state=ret_setting_state;
			}
